...
test('브라우저에서 글 생성·수정·삭제, 본문 이스케이프, 모바일', async ({ page, context, request }) => {
  ...
  await page.getByLabel('제목', { exact: true }).fill('브라우저 작성 글')
  await page.getByLabel('주소', { exact: true }).fill(slug)
  await page.getByLabel('본문', { exact: true }).fill('<script>alert("xss")</script>\n안녕하세요.')
  await page.getByRole('button', { name: '저장', exact: true }).click()
  await expect(page).toHaveURL(/\/admin\/posts$/)
  const row = page.getByRole('listitem').filter({ has: page.getByRole('heading', { name: '브라우저 작성 글', exact: true }) })
  await expect(row).toContainText('초안')
  expect((await request.get(`/api/posts/${slug}`)).status()).toBe(404)
  await row.getByRole('link', { name: '수정', exact: true }).click()
  await page.getByLabel('제목', { exact: true }).fill('수정한 글')
  await page.getByLabel('공개 상태').click()
  await page.getByRole('option', { name: '공개', exact: true }).click()
  await page.getByRole('button', { name: '저장', exact: true }).click()
  await expect(page).toHaveURL(/\/admin\/posts$/)
  await page.goto(`/posts/${slug}`)
  await expect(page.getByRole('heading', { name: '수정한 글' })).toBeVisible()
  ...
  const updated = page.getByRole('listitem').filter({ has: page.getByRole('heading', { name: '수정한 글', exact: true }) })
